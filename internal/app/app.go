...
import (
	...
	v1mw "github.com/JoshuaPangaribuan/dbot/internal/pkg/discord/v1/middleware"
...
func (a *application) setupGlobalMiddleware(bot *v1.Bot) {
	bot.EventBus().Use(
		v1.RequireGuild(),
		v1.IgnoreBot(),
		onlyEventTypes(
			v1mw.RateLimit(5, 10*time.Second),
			v1.EventTypeMessage,
			v1.EventTypeCommand,
			v1.EventTypeComponent,
			v1.EventTypeModal,
		),
	)
}

func (a *application) registerFeatures(bot *v1.Bot) {
	features := []struct {
		name string
		f    v1.Feature
	}{
...
func onlyEventTypes(mw v1.MiddlewareFunc, types ...v1.EventType) v1.MiddlewareFunc {
	allowed := make(map[v1.EventType]struct{}, len(types))
